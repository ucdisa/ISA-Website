    ...
    return (
        <div>
            <div className='hidden md:flex justify-between items-center py-[10px] gap-[20px] bg-[#f7f7f7] border-black border-[1px] px-[10px] rounded-md'>
                ...
                <p className={`rounded-md shadow-sm text-white text-[14px] px-[5px] py-[2px] flex justify-center items-center ${ticket.status == 'pending' ? 'bg-yellow-300' : ticket.status == 'approved' || ticket.status == "checked in" ? 'bg-green-400' : 'bg-red-400'}`}>{ticket.status == 'cancel' ? 'cancel requested' : ticket.status}</p>
